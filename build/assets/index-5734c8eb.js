...
const index = /* @__PURE__ */ getDefaultExportFromCjs(nanoassert$1);
const nanoassert$2 = /* @__PURE__ */ _mergeNamespaces({
  __proto__: null,
  default: index
}, [nanoassert$1]);
const require$$9 = /* @__PURE__ */ getAugmentedNamespace(nanoassert$2);
var assert$d = require$$9;
...
Trie.prototype.create = function(route) {
  ...
  function createNode(index2, trie2) {
    var thisRoute = has(routes, index2) && routes[index2];
    if (thisRoute === false)
      return trie2;
    var node2 = null;
    if (/^:|^\*/.test(thisRoute)) {
    ...
    return createNode(index2 + 1, node2);
  }
...
Nanobus.prototype.removeListener = function(eventName, listener) {
  ...
  function remove(arr, listener2) {
    if (!arr)
      return;
    var index2 = arr.indexOf(listener2);
    if (index2 !== -1) {
      splice(arr, index2, 1);
      return true;
    }
  ...
  return function(strings) {
    ...
    for (var i = 0; i < strings.length; i++) {
      if (i < arglen - 1) {
        ...
        parts.push.apply(parts, parse2(strings[i]));
    }
    var tree = [null, {}, []];
    var stack = [[tree, -1]];
...
if (window$1 && window$1.MutationObserver) {
  var observer = new MutationObserver(function(mutations) {
    ...
    for (var i = 0; i < mutations.length; i++) {
      ...
      eachMutation(mutations[i].removedNodes, function(index2, el) {
        if (!document$2.documentElement.contains(el))
          turnoff(index2, el);
      });
      eachMutation(mutations[i].addedNodes, function(index2, el) {
        if (document$2.documentElement.contains(el))
          turnon(index2, el);
      });
    }
...
Output.prototype.getTexture = function() {
  var index2 = this.pingPongIndex ? 0 : 1;
  return this.fbos[index2];
};
...
class HydraSource {
  ...
  initCam(index2, params) {
    const self2 = this;
    Webcam(index2).then((response) => {
...
(function(module, exports) {
  !function(r, t) {
    ...
    function f(r2, t2, e2) {
      for (var a3, n2 = new Float32Array(r2 + 2), o2 = new Float32Array(r2 + 2), i2 = t2 / 2, f2 = u(0), c2 = (u(i2) - f2) / (r2 + 1), s2 = new Array(r2 + 2), l2 = 0; l2 < n2.length; l2++)
    ...
    var y = Object.freeze({ __proto__: null, buffer: function(r2) {
      ...
      if ("object" != typeof t2)
        throw new TypeError();
      for (var a3 = t2, n2 = e2 / (2 * (a3.length - 1)), o2 = 0, i2 = 0; i2 < a3.length; i2++)
        o2 += a3[i2];
    ...
    var _ = {}, b = {}, M = { bitReverseArray: function(r2) {
      ...
      for (var n2 = M.bitReverseArray(e2), o2 = { real: [], imag: [] }, i2 = 0; i2 < e2; i2++)
...
class Audio {
  ...
  tick() {
    if (this.meyda) {
      ...
      if (features && features !== null) {
        ...
        this.bins = this.bins.map((bin, index2) => {
          return features.loudness.specific.slice(index2 * spacing, (index2 + 1) * spacing).reduce(reducer);
        }).map((bin, index2) => {
          return bin * (1 - this.settings[index2].smooth) + this.prevBins[index2] * this.settings[index2].smooth;
        });
        this.fft = this.bins.map((bin, index2) => (
          // Math.max(0, (bin - this.cutoff) / (this.max - this.cutoff))
          Math.max(0, (bin - this.settings[index2].cutoff) / this.settings[index2].scale)
        ));
        if (this.isDrawing)
          this.draw();
      }
  ...
  setBins(numBins) {
    ...
    this.bins.forEach((bin, index2) => {
      window["a" + index2] = (scale = 1, offset = 0) => () => a.fft[index2] * scale + offset;
    });
  }
  setScale(scale) {
    this.scale = scale;
  ...
  draw() {
    ...
    this.bins.forEach((bin, index2) => {
      ...
      this.ctx.fillRect(index2 * spacing, this.canvas.height - height, spacing, height);
      var y = this.canvas.height - scale * this.settings[index2].cutoff;
      this.ctx.beginPath();
      this.ctx.moveTo(index2 * spacing, y);
      this.ctx.lineTo((index2 + 1) * spacing, y);
      this.ctx.stroke();
      var yMax = this.canvas.height - scale * (this.settings[index2].scale + this.settings[index2].cutoff);
      this.ctx.beginPath();
      this.ctx.moveTo(index2 * spacing, yMax);
      this.ctx.lineTo((index2 + 1) * spacing, yMax);
      this.ctx.stroke();
    });
  }
}
...
const ArrayUtils = {
  ...
  getValue: (arr = []) => ({ time, bpm }) => {
    ...
    let index2 = time * speed * (bpm / 60) + (arr._offset || 0);
    if (smooth !== 0) {
      let ease = arr._ease ? arr._ease : easing["linear"];
      let _index = index2 - smooth / 2;
      let currValue = arr[Math.floor(_index % arr.length)];
      let nextValue = arr[Math.floor((_index + 1) % arr.length)];
      let t = Math.min(_index % 1 / smooth, 1);
      return ease(t) * (nextValue - currValue) + currValue;
    } else {
      arr[Math.floor(index2 % arr.length)];
      return arr[Math.floor(index2 % arr.length)];
    }
  }
};
...
function formatArguments(transform2, startIndex, synthContext) {
  ...
  return defaultArgs.map((input, index2) => {
    const typedArg = {
      value: input.default,
      type: input.type,
      //
    ...
    if (userArgs.length > index2) {
      typedArg.value = userArgs[index2];
      if (typeof userArgs[index2] === "function") {
        typedArg.value = (context, props, batchId) => {
          try {
            const val = userArgs[index2](props);
            if (typeof val === "number") {
              return val;
            } else {
              console.warn("function does not return a number", userArgs[index2]);
            }
      ...
      } else if (userArgs[index2].constructor === Array) {
        typedArg.value = (context, props, batchId) => ArrayUtils.getValue(userArgs[index2])(props);
        typedArg.isUniform = true;
      }
    }
    if (startIndex < 0)
...
class HydraRenderer {
  ...
  _initOutputs(numOutputs) {
    const self2 = this;
    this.o = Array(numOutputs).fill().map((el, index2) => {
      var o = new Output({
        regl: this.regl,
        width: this.width,
        height: this.height,
